...
class fastbm25(object):
    ...
    def __init__(self, corpus, PARAM_K1=1.5, PARAM_B=0.75, EPSILON=0.25):
        """
        Parameters
        ----------
    ...
    def similarity_bm25(self, document_a, document_b):
        ...
        for word in document_a:
            if word not in doc_freqs:
                continue
            score += (self.idf.get(word,default_idf) * doc_freqs[word] * (self.PARAM_K1 + 1)
                      / (doc_freqs[word] + self.PARAM_K1 * (1 - self.PARAM_B + self.PARAM_B * len(document_b) / self.avgdl)))
        return score
    ...
    def get_score_by_reversed_index_all_documents(self, corpus):
        ...
        for index, document in enumerate(corpus):
            ...
            for word in document:
                if word not in doc_freqs:
                    continue
                score = (self.idf[word] * doc_freqs[word] * (self.PARAM_K1 + 1)
                          / (doc_freqs[word] + self.PARAM_K1 * (1 - self.PARAM_B + self.PARAM_B * self.doc_len[index] / self.avgdl)))
